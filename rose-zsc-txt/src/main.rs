...
fn main() -> Result<(), ::std::io::Error> {
    ...
    if !stb_path.exists() {
        bail(&format!("STB file does not exist: {}", stb_path.to_string_lossy()));
    }

    let zsc_path = PathBuf::from(&args[1]);
    if !zsc_path.exists() {
        bail(&format!("ZSC file does not exist: {}", zsc_path.to_string_lossy()));
    }
    ...
    if stb_col > stb.cols() as i32 {
        bail(&format!("STB column exceeds column count: {} ({} max)", stb_col, stb.cols()));
    }

    for (object_idx, stb_row) in stb.data.iter().enumerate() {
        let txt_file = &stb_row[stb_col as usize];
        if txt_file.is_empty() {
            println!("No txt file name in column {} for row {}. Skipping.", stb_col, object_idx);
            continue;
        }
        ...
        for (part_idx, part) in zsc_obj.parts.iter().enumerate() {
            let mesh = &zsc.meshes[part.mesh_id as usize];
            let mat = &zsc.materials[part.material_id as usize];
            ...
            writeln!(f, "\tpos {} {} {}", part.position.x, part.position.y, part.position.z)?;
            writeln!(f, "\trot {} {} {} {}", part.rotation.w, part.rotation.x, part.rotation.y, part.rotation.z)?;
            writeln!(f, "\tscale {} {} {}", part.scale.x, part.scale.y, part.scale.z)?;
        ...
        for (effect_idx, effect) in zsc_obj.effects.iter().enumerate() {
            let effect_path = &zsc.effects[effect.effect_id as usize];
            ...
            writeln!(f, "\tpos {} {} {}", effect.position.x, effect.position.y, effect.position.z)?;
            writeln!(f, "\trot {} {} {} {}", effect.rotation.w, effect.rotation.x, effect.rotation.y, effect.rotation.z)?;
            writeln!(f, "\tscale {} {} {}", effect.scale.x, effect.scale.y, effect.scale.z)?;
        }
    }
